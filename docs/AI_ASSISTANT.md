...
- **Anonymous single-break gating (webapp).** Only active when suite auth is configured (`isSuiteAuthConfigured()`), so standalone/OSS deployments stay fully open. A signed-out user gets one non-persisted break: cloud save (already entitlement-gated) and share-link prompt sign-in; a standing notice explains the break isn't saved and clears on reload.
