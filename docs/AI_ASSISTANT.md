...
## 4b. Field reality check ("Col") — ✅ built (2026-08-18)

**Origin:** an owner-run persona exercise (a sceptical, forty-year heavy-plant/hand-crew veteran critiquing the app) surfaced a real product gap — the app's credibility depends on someone experienced sanity-checking a desk-planned estimate before it reaches a crew, and that person isn't always available. The fix bakes that reality-check *perspective* into the tool itself, so a field veteran doesn't have to be present for every planning decision — while still deferring the field verification itself to the actual field (see `master_plan.md`'s "Not built this pass" note below for what stays a genuine, unclosed gap).

- **`POST /api/assistant/reality-check`** (`assistantRealityCheck.ts`) — same always-something-useful contract as `assistant/briefing`: a validated AI critique under the identical grounding contract (`buildRealityCheckSystemPrompt`, `aiGrounding.ts` — the STRICT RULES block is shared verbatim with the narration prompt, not a looser copy) when the model is configured and stays grounded; otherwise a deterministic template (`realityCheckTemplate.ts`) built straight from the payload's own honesty flags. **The template path is real critique, not a stub** — it never needs a live model to flag a genuine gap, because the gaps themselves (estimated data, low-confidence segments, an unverified standard-item sourceNote, mobilisation not costed) are already computed deterministically; the AI layer only adds plain-language framing on top.
- **Reality-check context** — `AssistantPayload` gained four optional fields (`segmentCount`, `lowConfidenceSegmentCount`, `equipmentCaveats`, `mobilisationCostIncluded`), a must-match pair (`api/src/types/assistant.ts` ↔ `webapp/src/utils/assistantApi.ts`) so the persona has real ammunition instead of a vague description: `equipmentCaveats` is populated by joining the tasked equipment against the effective catalogue (overrides applied) and pulling each standard item's own `sourceNote` — the SAME accuracy-review citations the equipment panel's "Std" badge tooltip shows (`CALCULATION_REVIEW.md` "Standard catalogue accuracy review").
- **UI:** `RealityCheckCard.tsx`, mounted in the Assistant tab alongside (never merging into) `AiAssistantCard.tsx` — distinct amber/hi-vis styling, its own source badge, its own citation chips. A button shows the flag count before running ("Run reality check (2 flags to check)") so a user can see there's something worth checking without spending a model call first.
- **Not yet built:** the reality check reads the plan's own honesty flags; it has no independent field-verification data of its own (no job-actuals log to compare against — see roadmap). It is a sharper *desk* review, not a substitute for the field check it recommends.
