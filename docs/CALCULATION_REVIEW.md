...
- **A1 — De-duplication:** the accurate model now lives **only** in the backend; the frontend delegates to it. **Correction, 2026-08-18:** "the frontend fallback remains solely as a degraded offline path" (as originally written here) was **not accurate** — `AnalysisPanel.tsx`'s legacy coarse single-bucket model (`terrainFactors`/`vegetationFactors`) was being computed and displayed during the ordinary async window before every backend response resolved, not just when the backend was genuinely unreachable, so the shown estimate could silently swap between two materially different numbers depending on network timing. Fixed: the frontend calculation now only runs (and only feeds `finalCalculations`) when the backend is confirmed unavailable or has errored — never merely "hasn't answered yet." See the 2026-08-18 entry below.
...
- **A2/F4 — Server-side elevation profile:** `api/src/services/elevationService.ts` + `POST /api/elevation/profile` sample a bare-earth DEM (ArcGIS `getSamples`, configurable `DEM_IMAGESERVER_URL`) in **one request per line**; `slopeCalculation.ts` batches through it and falls back to Terrain-RGB. Removes the per-point tile-decode chattiness when a DEM is configured. **2026-08-17:** the API caps a single request at 5,000 points (a real upstream constraint — every point is encoded into the `getSamples` request URL); `sampleElevationsBatch` now chunks a larger point set into multiple ≤5,000-point requests instead of falling back to per-point Terrain-RGB for the whole set the moment it didn't fit in one request — see `ROUTE_INTELLIGENCE.md`'s "Data cost" section for the Terrain Mobility production incident this was found from.
...
**2026-08-18 — mobility-style efficiency/accuracy pass over fire-break mode, plus water/trail visualization.** A deep review of the live code (not a re-check of the F1–F8/A1–A7 items above, which this confirmed are still in place) found five real issues:

- **Manual-draw vegetation analysis wasn't using the area-batching the codebase had already built.** `routeOptimizer.ts`'s `sampleVegetation` pre-warms a shared area cache (`fetchStateVegetationArea` — at most one NSW polygon query + one NVIS raster export per corridor) before falling back to per-point queries, but `vegetationAnalysis.ts`'s `analyzeTrackVegetation` — the primary manual-draw fire-break flow — never called it, so every drawn line fired one point query per ~200 m sample against the same government ArcGIS servers the code's own comments warn would be "overwhelm[ed]... at any real scale." Fixed: `analyzeTrackVegetation` now pre-warms the same area cache (above the same `AREA_QUERY_MIN_POINTS = 24` threshold `routeOptimizer.ts` uses) alongside its existing corridor-wide water/trail fetch, so the per-segment `fetchStateVegetation` calls that follow resolve locally.
- **Slope and vegetation analysis ran sequentially despite being independent.** `MapboxMapView.tsx`'s `analyzeAndRender` awaited `analyzeTrackSlopes` then `analyzeTrackVegetation` back-to-back (different data sources — DEM vs NVIS/NSW/Mapbox — with no dependency between them). Now `Promise.all`'d; vegetation failure stays non-fatal exactly as before.
- **The A1 "split-brain" was still live in production, not just an offline fallback** — see the correction on A1 above. Fixed by gating `AnalysisPanel.tsx`'s frontend calculation on the backend being confirmed unavailable/errored, never merely pending. This also stops the frontend's full per-equipment loop (with per-item debug logging) from running on every render whose result would be thrown away — an efficiency fix riding along with the accuracy one.
- **Cross-slope probe resolution looped a `Promise.all` per probe instead of batching a segment's probes together.** `slopeCalculation.ts`'s along-line elevation resolution already batches every profile point in one `Promise.all`; the cross-slope probe loop two lines below it didn't, so a cold DEM cache serialised what should have been one batched Terrain-RGB fetch per segment. Fixed to match the along-line pattern.
- **An unsourced `refRate × 0.1` over-limit fallback rate had no name or rationale comment**, unlike every other constant in `equipmentAnalysis.ts`/`productionModel.ts`. Named `OVER_LIMIT_NOTIONAL_RATE_FRACTION` with a comment stating plainly that it remains an unsourced, deliberately-conservative placeholder — same honesty standard as F3/F7, not a claim it's now grounded.

**Follow-on (same day): water and existing-trail visualization, ported from Terrain Mobility's own hydrology reference layer.** Fire-break mode already fetches real OSM water and reusable-trail geometry per line (the source of the `isWater`/`onExistingTrail` flags above) but discarded the geometry itself once the per-segment booleans were derived — a user could see "existing trail reused: 340 m" in a text note with no way to see *which* 340 m. `VegetationAnalysis` now carries `waterFeatures`/`trailFeatures` (the same `{kind, coords}` shape Terrain Mobility's `waterFeatures` prop already uses); `MapboxMapView.tsx` draws them with the identical technique as Terrain Mobility's hydrology layer (§34) — filled polygon for water bodies, cased line for rivers/streams, a distinct dashed cased line for existing trails/tracks (styled as reference, not recommendation, matching its informational-only status). Repurposed the component's previously-dead `vegetationAnalysis` state slot rather than adding a new one.

Gates green: api `npm run build` + `npm run test:unit`; webapp `npm run build` + `npm test` (46/46).
