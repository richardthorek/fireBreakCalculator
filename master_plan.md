...
| 48 (OCOKA 1) | Mobility-class vocabulary migration — one MCOO vocabulary instead of two | New `terrain/mobilityClass.ts`: canonical `MobilityClass = 'unrestricted' \| 'restricted' \| 'severely-restricted'`, `fromLegacy()`/`toLegacy()`, `SEVERELY_RESTRICTED_MEANING` (the doctrinal-disambiguation string legend/export/briefing all now share, since this engine's `severely-restricted` is a hard gate and doctrine's is not). `mobilityCost.ts`'s `TrafficabilityClass` and `corridorField.ts`'s `CorridorEaseClass` both collapse onto it (`'open'` → `'unrestricted'`); `Corridor.goFraction`/`slowGoFraction`/`noGoFraction` → `unrestrictedFraction`/`restrictedFraction`/`severelyRestrictedFraction`; `MobilityAppreciationResult.noGoCount`/`slowGoCount` → `severelyRestrictedCount`/`restrictedCount`. Map paint expression, CSS (`.corridor-ease--unrestricted`), legend and panel labels updated. Three contract risks handled explicitly: **GIS export** dual-emits current and legacy field names for one release (`schema_version: 2`, docs §29 addendum); **`MobilityAssistantPayload`** (the known must-match webapp/api pair, two prior confirmed hits) renamed both sides with the API validator's new `normalizeLegacyMobilityFields()` accepting a cached client's old field names in place, proven by a dedicated test; **`mobilityTelemetry.ts`'s wire field names stay frozen** (`noGoCount`/`slowGoCount`/`goCount`) — an existing Table Storage analytics series the OCOKA 5/8 backend-routing threshold depends on — mapped explicitly at the recording boundary instead of renamed. Closed a real gap found along the way: `webapp/tests/`'s 33 files ran by hand only (`tsx` wasn't even a devDependency, CI never ran them) — added `scripts/runTests.mjs` + `npm test`, wired into `deploy.yml` as a real CI gate, with the one pre-existing live-network test (`nvis-fidelity.test.ts`) explicitly excluded rather than silently tolerated. Verified: `npm test` (33/33), `npm run build` (webapp), `npm run test:unit` (api) all clean | [ROUTE_INTELLIGENCE.md](docs/ROUTE_INTELLIGENCE.md) §47 |
...
- ~~**OCOKA 1 — mobility-class vocabulary migration**~~ — **Shipped, step 48.** `mobilityClass.ts` built as designed; all three "must NOT be renamed" items (telemetry wire names, `--tac-*` CSS tokens, no saved-plan risk) held; the `SEVERELY_RESTRICTED_MEANING` honesty catch shipped as an exported constant, not just prose, so legend/export/briefing share one string. Extra ground covered while touching this: `webapp/tests/` (33 files) wired into CI for the first time via `npm test`.
...
- **2026-08-02 — OCOKA 1 shipped (step 48): mobility-class vocabulary
  migration.** First code increment of the OCOKA programme. Collapsed
  `mobilityCost.ts`'s `GO/SLOW-GO/NO-GO` and `corridorField.ts`'s
  `open/restricted/severely-restricted` onto one canonical `MobilityClass`
  union in new `terrain/mobilityClass.ts`, threaded through the search
  (`accumulatedCost.ts`), corridor aggregation and `riskScore`, the map paint
  expression, CSS, the panel and legend, `mobilityAppreciation.ts`'s result
  object, the GIS exporter, and both sides of the `MobilityAssistantPayload`
  must-match pair. Three contract risks handled rather than assumed away: GIS
  export dual-emits current + legacy field names for one release
  (`schema_version: 2`); the assistant payload validator gained
  `normalizeLegacyMobilityFields()`, accepting a cached client's old field
  names in place, proven by a dedicated test simulating exactly that client;
  `mobilityTelemetry.ts`'s wire names stay frozen (`noGoCount`/`slowGoCount`/
  `goCount`) since that's an existing Table Storage analytics series, mapped
  explicitly at the recording boundary instead of renamed. Shipped the
  doctrinal disambiguation as an exported constant
  (`SEVERELY_RESTRICTED_MEANING`) rather than scattered prose, since this
  engine's `severely-restricted` is a hard gate and the doctrinal term isn't.
  Found and closed a real gap while touching the surface it affects:
  `webapp/tests/`'s 33 test files ran by hand only (`tsx` wasn't even a
  devDependency, CI never ran them) — added `scripts/runTests.mjs` + `npm
  test`, now a real CI gate in `deploy.yml`, with the one pre-existing
  live-network test (`nvis-fidelity.test.ts`) explicitly excluded by name
  rather than silently tolerated as a mystery failure. All three gates green:
  `npm test` (33/33), `npm run build` (webapp), `npm run test:unit` (api).

- **2026-08-02 — Terminology self-correction: OAKOC/IPOE → OCOKA/IPB.** The
  entry immediately below researched only US Army doctrine (ATP 2-01.3) and
  recommended OAKOC/IPOE as "current", without checking what the ADF —
  `PITCH_TERRAIN_DENIAL.md`'s actual named audience (NORFORCE, RFSU, 1CER,
  Pilbara Regiment) — currently teaches. Corroborated across multiple searches
  against The Cove (the Australian Army's own professional-military-education
  platform): the **Australian Army currently uses OCOKA and IPB**, not the
  reordered OAKOC/renamed IPOE the US Army moved to. These are two different
  armies' current terminology, not an old-vs-new supersession — the earlier
  entry's framing of OCOKA as simply "superseded" was wrong for this product's
  audience. All roadmap items renumbered `OAKOC N` → `OCOKA N` (stage content
  unchanged — the five factors and their definitions are identical under
  either ordering, only the acronym, the IPB/IPOE label and the panel's
  section order change), and the doctrinal-reference text in
  `CLAUDE.md`/`ROUTE_INTELLIGENCE.md` §47 corrected to match. Residual
  uncertainty stated plainly: this is corroborated via search snippets of one
  source family, not a document read in full (`cove.army.gov.au` returned 503
  on every direct fetch attempted), and no specific current LWP-G/LWD
  publication was located confirming OCOKA as still doctrinally mandated
  rather than merely commonly taught — a firmer primary source or SME review
  remains worth doing before this goes in front of a serving audience, per
  `PITCH_TERRAIN_DENIAL.md`'s own closing note.

- **2026-08-02 — OCOKA programme added to the roadmap (stages 1–9)** [terminology
  in this entry was corrected same-day, see above]: owner asked how the mode's
  "inadvertently implemented" military terrain framework could be used to
  redefine analysis and presentation, and separately directed the compute onto
  a parallel backend with a ~10 s first-paint / ~10 s update contract. Research
  finding at the time (see correction above): the acronym the work started
  from, **OCOKA**, was believed to be the superseded form (FM 34-130, 1994) in
  favour of US doctrine's **OAKOC**, with the parent process renamed from
