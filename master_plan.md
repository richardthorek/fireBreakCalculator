...
| 3 | **National live context** | National situational layers — DEA Hotspots + Digital Atlas NRT bushfire boundaries (both official, national, CORS-clean); jurisdictional incident/warning overlay (AWS-keyed) as an uplift. **Endpoints confirmed 2026-07-11.** | [GIS_INTEROP.md](docs/GIS_INTEROP.md) §4 | 📋 |
| 3a | **AFDRS fire danger** | Official fire danger rating + behaviour index for plan location/date; break-adequacy heuristics keyed to AFDRS, doctrine-cited — **display official product only, no spread rebuild** | [GIS_INTEROP.md](docs/GIS_INTEROP.md) §4 | 📋 next task |
