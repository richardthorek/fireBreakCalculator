...
- **Movement-analysis performance + progressive-painting programme** (Difficulty: M/L, in progress)
  ...
  - Progress so far, on `claude/movement-analysis-perf-u1yhy5` (no PR opened yet): map layers now update incrementally instead of rebuilding; a precomputed per-grid search index removes redundant work across a run's ~120 Dijkstra passes (≈2.2–2.5× on the search core alone), with two real correctness defects found and fixed via mutation testing during review, not caught by an initial "tests pass" claim; corridor-field construction and both min-cut solves moved off the main thread, which is the actual hang fix; mover-ensemble transit cells now stream to the map as movers complete (real interim data, not synthetic), and key-terrain candidate scoring — the single largest chunk of a run's Dijkstra passes — now fans out across multiple worker instances.
  - Remaining: redundant-pass elimination (min-cut computing edge costs twice, `applyCrossSlope` recomputing the whole grid every lazy-grid round instead of just new cells); the coarse-to-fine search itself; frontier-streaming from the Dijkstra search phase.
